			...
			onload: function (response) {
				if (response.status === 200) {
					...
					if (match) {
						...
						console.error(
							`JSON-like structure not found in response for user ID: ${userID}`,
						)
					}
				} else {
					console.error(
						`Error fetching user profile for user ID: ${userID}. Status: ${response.status}`,
					)
				}
			},
			onerror: function (error) {
				console.error(
					`Error fetching user profile for user ID: ${userID}`,
					error,
				)
			},
		})
